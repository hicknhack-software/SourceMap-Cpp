...
foreach ($file in $files) {
    ...
    $content = $content -replace '/\* SourceMap-Qt', '/* SourceMap-Cpp'
    $content = $content -replace '\(C\) Copyright \d+ HicknHack Software GmbH', '(C) Copyright 2025 HicknHack Software GmbH'
    $content = $content -replace 'https://github.com/hicknhack-software/SourceMap-Qt', 'https://github.com/hicknhack-software/SourceMap-Cpp'
